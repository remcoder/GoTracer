...
class App {
  ...
  constructor() {
    ...
    const initialUrl = this.getQueryParam('image');
    if (initialUrl) {
    ...
    const initialUrlOnLoad = this.getQueryParam('image');
    if (initialUrlOnLoad) {
  ...
  update = () => {
    ...
    const hashCorners = this.goTracer.corners.map(c => (isNaN(c.x) || isNaN(c.y) ? '' : c.toString())).join(',');
  ...
  downloadSGF = () => {
    ...
    const sgfContent = this.goTracer.getSGF();
    if (!sgfContent) {
